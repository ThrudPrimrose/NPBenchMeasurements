#!/usr/bin/env python3

import argparse
import pathlib
import sqlite3
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
import seaborn as sns
from scipy.stats import pearsonr, spearmanr

CACHE_LINE = 64  # bytes
WRITE_ALLOCATE = 1
WORD_SIZE = 4 #word=2, dword=4, qword=8, assume most loads to be 4? 

def read_sqlite_db(db_path: pathlib.Path):
    print(f"\n=== Database: {db_path} ===")

    conn = sqlite3.connect(db_path)
    dfs = dict()
    try:
        tables = pd.read_sql(
            "SELECT name FROM sqlite_master WHERE type='table';", conn
        )["name"].tolist()

        if not tables:
            print("  (No tables found)")
            return

        for table in tables:
            df = pd.read_sql(f"SELECT * FROM {table}", conn)
            dfs[table] = df
    finally:
        conn.close()
    
    return dfs


def analyze_metric_closeness(df: pd.DataFrame):
    """
    Analyze which metric is closest to Symbolic Bytes
    """
    # Metrics to compare against Symbolic Bytes
    metric_columns = [
        'L3 Misses',
        'L3 DRead + DWrite',
        'L3 DMisses',
        'L3 LST Miss',
        'L3 Access',
        'Max(L1, L2, L3) Misses',
        'LST',
    ]
    
    # Filter out rows where Symbolic Bytes is 0 or NaN
    df_filtered = df[df['Symbolic Bytes'] > 0].copy()
    
    if len(df_filtered) == 0:
        print("Warning: No valid data for correlation analysis")
        return None, None
    
    results = {}
    
    for metric in metric_columns:
        # Calculate various error metrics
        symbolic = df_filtered['Symbolic Bytes'].values
        measured = df_filtered[metric].values
        
        # Skip if metric has all zeros
        if np.all(measured == 0):
            continue
        
        # Pearson correlation
        pearson_corr, _ = pearsonr(symbolic, measured)
        
        # Spearman correlation (rank-based, more robust)
        spearman_corr, _ = spearmanr(symbolic, measured)
        
        # Mean Absolute Percentage Error (MAPE)
        mape = np.mean(np.abs((symbolic - measured) / symbolic)) * 100
        
        # Relative error (how much does metric overestimate/underestimate)
        relative_error = np.mean((measured - symbolic) / symbolic) * 100
        
        # R-squared (coefficient of determination)
        ss_res = np.sum((symbolic - measured) ** 2)
        ss_tot = np.sum((symbolic - np.mean(symbolic)) ** 2)
        r_squared = 1 - (ss_res / ss_tot) if ss_tot > 0 else 0
        
        # Root Mean Square Error
        rmse = np.sqrt(np.mean((symbolic - measured) ** 2))
        
        results[metric] = {
            'Pearson Correlation': pearson_corr,
            'Spearman Correlation': spearman_corr,
            'R-squared': r_squared,
            'MAPE (%)': mape,
            'Mean Relative Error (%)': relative_error,
            'RMSE': rmse,
        }
    
    if not results:
        print("Warning: No valid metrics for correlation analysis")
        return None, None
    
    # Convert to DataFrame for easy viewing
    results_df = pd.DataFrame(results).T
    results_df = results_df.sort_values('Pearson Correlation', ascending=False)
    
    print("\n" + "=" * 90)
    print("METRIC CLOSENESS TO SYMBOLIC BYTES")
    print("=" * 90)
    print(results_df.to_string())
    print("\n")
    
    # Find the best metric
    best_metric = results_df.index[0]
    print(f"✓ Best metric (highest Pearson correlation): {best_metric}")
    print(f"  Pearson Correlation: {results_df.loc[best_metric, 'Pearson Correlation']:.4f}")
    print(f"  R-squared: {results_df.loc[best_metric, 'R-squared']:.4f}")
    print(f"  MAPE: {results_df.loc[best_metric, 'MAPE (%)']:.2f}%")
    print("=" * 90 + "\n")
    
    return results_df, best_metric


def main():
    parser = argparse.ArgumentParser(
        description="Read SQLite database files into pandas and print them"
    )
    parser.add_argument(
        "--path",
        type=pathlib.Path,
        default=pathlib.Path("../npbench_papi_intel_xeon_6154/L/npbench_papi_metrics.db"),
        help="Path to a SQLite file or a directory containing SQLite files",
    )
    parser.add_argument(
        "--pattern",
        default="*.db",
        help="Filename pattern to match (default: *.db)",
    )
    parser.add_argument(
        "--preset",
        default="L",
        help="Preset to filter benchmarks (default: L)",
    )

    args = parser.parse_args()

    if args.path.is_file():
        dfs = read_sqlite_db(args.path)
    elif args.path.is_dir():
        raise Exception("Directory input not supported in this version.")

    df = dfs["event_averages"]

    # =========================
    # PAPI METRICS – SEMANTICS
    # =========================

    # PAPI_DP_OPS
    #   Double-precision floating-point operations retired.
    #   Used as FLOPs in roofline models.
    #
    #   FLOPs = PAPI_DP_OPS


    # -------------------------
    # Load / Store Instructions
    # -------------------------

    # PAPI_LD_INS
    #   Number of retired load instructions.
    #   Counts instructions, NOT bytes.
    #   Not suitable alone for data movement estimation.

    # PAPI_SR_INS
    #   Number of retired store instructions.
    #   Store instructions may trigger write-allocate traffic.

    # PAPI_LST_INS
    #   Total load + store instructions.


    # -------------------------
    # L1 Cache Misses
    # -------------------------

    # PAPI_L1_DCM
    #   L1 data cache misses.
    #   Each miss implies a cache-line transfer from L2.
    #
    #   Bytes_L2 ≈ PAPI_L1_DCM * CACHE_LINE_SIZE

    # PAPI_L1_ICM
    #   L1 instruction cache misses (ignore for data movement).

    # PAPI_L1_TCM
    #   Total L1 cache misses (data + instruction).

    # PAPI_L1_LDM
    #   L1 load misses.

    # PAPI_L1_STM
    #   L1 store misses (affected by write-allocate policy).


    # -------------------------
    # L2 Cache Misses
    # -------------------------

    # PAPI_L2_DCM
    #   L2 data cache misses.
    #   Each miss implies a cache-line transfer from L3.

    # PAPI_L2_ICM
    #   L2 instruction cache misses.

    # PAPI_L2_TCM
    #   Total L2 cache misses.
    #
    #   Bytes_L3 ≈ PAPI_L2_TCM * CACHE_LINE_SIZE

    # PAPI_L2_LDM
    #   L2 load misses.

    # PAPI_L2_STM
    #   L2 store misses.


    # -------------------------
    # L3 Cache Misses (DRAM Traffic)
    # -------------------------

    # PAPI_L3_TCM
    #   Total L3 cache misses.
    #   Each miss fetches one cache line from main memory (DRAM).
    #
    #   Bytes_DRAM ≈ PAPI_L3_TCM * CACHE_LINE_SIZE
    #
    #   OperationalIntensity = PAPI_DP_OPS / Bytes_DRAM

    # PAPI_L3_LDM
    #   L3 load misses.


    # -------------------------
    # Cache Accesses (Hits + Misses)
    # -------------------------

    # PAPI_L2_DCA
    #   L2 data cache accesses (hits + misses).
    #   L2 hit rate:
    #     L2_hit_rate = 1 - (PAPI_L2_DCM / PAPI_L2_DCA)

    # PAPI_L3_DCA
    #   L3 data cache accesses.

    # PAPI_L2_TCA
    #   Total L2 cache accesses.

    # PAPI_L3_TCA
    #   Total L3 cache accesses.


    # -------------------------
    # Cache Reads / Writes
    # -------------------------

    # PAPI_L2_DCR
    #   L2 data cache reads.

    # PAPI_L2_DCW
    #   L2 data cache writes.

    # PAPI_L3_DCR
    #   L3 data cache reads.

    # PAPI_L3_DCW
    #   L3 data cache writes.
    #
    #   Read/Write-aware L3 traffic model:
    #     Bytes_L3 ≈ (PAPI_L3_DCR + WRITE_ALLOCATE * PAPI_L3_DCW) * CACHE_LINE_SIZE
    #
    #   Where WRITE_ALLOCATE = 1 on most x86 CPUs.


    # -------------------------
    # Cache Coherency (Multicore)
    # -------------------------

    # PAPI_CA_SNP
    #   Cache snoop requests.

    # PAPI_CA_SHR
    #   Shared cache-line requests.

    # PAPI_CA_CLN
    #   Clean cache-line requests.

    # PAPI_CA_ITV
    #   Cache invalidations.
    #
    #   Usually ignored for single-thread or socket-local roofline models.


    # =========================
    # COMMON DERIVED METRICS
    # =========================

    # CACHE_LINE_SIZE = 64  # bytes (x86 / AMD EPYC / Intel)

    # Bytes moved at each level:
    #   Bytes_L2   = PAPI_L1_DCM * CACHE_LINE_SIZE
    #   Bytes_L3   = PAPI_L2_TCM * CACHE_LINE_SIZE
    #   Bytes_DRAM = PAPI_L3_TCM * CACHE_LINE_SIZE

    # Roofline-safe upper bound (avoids double counting):
    #   TotalBytes = max(PAPI_L1_DCM, PAPI_L2_TCM, PAPI_L3_TCM) * CACHE_LINE_SIZE

    # Bandwidth:
    #   BW = TotalBytes / execution_time
    # Assuming df is your 'event_averages' table
    # Pivot table so we can access metrics by event_name per benchmark
    pivot = df.pivot_table(
        index="benchmark",
        columns="event_name",
        values="average",
        aggfunc="first"  # in case duplicates exist
    ).fillna(0.0)

    # Time per benchmark (assuming same time per group)
    time_per_bench = df.groupby("benchmark")["time"].first()

    # Function to compute all traffic models per row
    def compute_traffic(row, time_sec):
        l1_dcm = row.get("PAPI_L1_DCM", 0.0)
        l2_tcm = row.get("PAPI_L2_TCM", 0.0)
        l3_tcm = row.get("PAPI_L3_TCM", 0.0)
        l3_ldm = row.get("PAPI_L3_LDM", 0.0)
        l3_stm = row.get("PAPI_L3_STM", 0.0)
        l3_dcr = row.get("PAPI_L3_DCR", 0.0)
        l3_dcw = row.get("PAPI_L3_DCW", 0.0)
        l3_tca = row.get("PAPI_L3_TCA", 0.0)
        dp_ops = row.get("PAPI_DP_OPS", 0.0)
        lst = row.get("PAPI_LST_INS", 0.0)
        flops = row.get("PAPI_FP_OPS", 0.0) + row.get("PAPI_DP_OPS", 0.0)

        # Data movement estimates
        # Formulas to try:
        bytes_dram_min   = l3_tcm * CACHE_LINE
        bytes_dram_rw    = (l3_dcr + l3_dcw) * CACHE_LINE
        bytes_dram_upper = l3_tca * CACHE_LINE
        bytes_hier       = max(l1_dcm, l2_tcm, l3_tcm) * CACHE_LINE
        bytes_dram_wb    = (l3_tcm) * CACHE_LINE
        bytes_dmiss      = (l3_ldm * 2) * CACHE_LINE

        bytes_lst = lst * WORD_SIZE

        def safe_div(a, b):
            return a / b if b > 0 else float("nan")

        # Operational intensity
        oi_min = safe_div(dp_ops, bytes_dram_min)
        oi_rw  = safe_div(dp_ops, bytes_dram_rw)
        oi_wb  = safe_div(dp_ops, bytes_dram_wb)

        # Bandwidth (bytes/sec)
        bw_min = safe_div(bytes_dram_min, time_sec)
        bw_rw  = safe_div(bytes_dram_rw, time_sec)
        bw_wb  = safe_div(bytes_dram_wb, time_sec)

        return pd.Series({
            "L3 Misses": bytes_dram_min,
            "L3 DRead + DWrite": bytes_dram_rw,
            "L3 DMisses": bytes_dram_wb,
            "L3 LST Miss": bytes_dmiss,
            "L3 Access": bytes_dram_upper,
            "Max(L1, L2, L3) Misses": bytes_hier,
            "LST": bytes_lst,
            "PAPI FLOPs": flops,
        })

    # Apply per benchmark
    results = pivot.apply(lambda row: compute_traffic(row, time_per_bench[row.name]), axis=1)

    # Merge with pivot for inspection if needed
    results = results.reset_index()

    symbolic_analysis = pd.read_csv("volumes_per_preset.csv")
    preset = args.preset
    # Filter symbolic_analysis for the current preset
    symbolic_filtered = symbolic_analysis[symbolic_analysis["preset"] == preset]

    # Merge with results on benchmark/kernel
    results = results.merge(
        symbolic_filtered,
        left_on="benchmark",
        right_on="kernel",
        how="left"
    )

    # Drop Na (at least one analysis failed)
    results = results.dropna(subset=["preset"])

    # Sum up read and write
    results["Symbolic Bytes"] = (
        results["symbolic_volume_read_bytes"] + 
        results["symbolic_volume_write_bytes"]
    )
    print(results)

    # Save the dataframe to CSV
    results.to_csv("results.csv", index=False)
    # Connect (or create) a SQLite database file
    conn = sqlite3.connect("results.db")
    # Save the DataFrame as a table named 'results'
    results.to_sql("results", conn, if_exists="replace", index=False)
    # Close the connection
    conn.close()

    # Analyze correlations BEFORE plotting
    correlation_results, best_metric = analyze_metric_closeness(results)

    # Pass correlation results to plotting functions
    plot_memory(results, correlation_results, best_metric)
    plot_flops(results)
    
    if correlation_results is not None:
        plot_correlation_analysis(results, correlation_results)
        plot_correlation_summary(correlation_results)


def plot_memory(df: pd.DataFrame, correlation_results=None, best_metric=None):
    """Original scatter plot per benchmark"""
    
    # Traffic columns (exclude OI)
    traffic_columns = [
        'L3 Misses',
        'L3 DRead + DWrite',
        'L3 DMisses',
        'L3 LST Miss',
        'L3 Access',
        'Max(L1, L2, L3) Misses',
        'LST',
        "Symbolic Bytes",
    ]

    # Melt the dataframe for plotting
    df_melt = df.melt(
        id_vars=["benchmark"], 
        value_vars=traffic_columns,
        var_name="Variant",
        value_name="Bytes_Moved"
    )

    # Plot setup
    sns.set(style="whitegrid")
    benchmarks = df["benchmark"].unique()
    n_cols = 3
    n_rows = (len(benchmarks) + n_cols - 1) // n_cols

    fig, axes = plt.subplots(n_rows, n_cols, figsize=(5*n_cols, 4*n_rows), squeeze=False)

    for i, benchmark in enumerate(benchmarks):
        ax = axes[i // n_cols, i % n_cols]
        data = df_melt[df_melt["benchmark"] == benchmark]

        # Scatter plot: x = variant, y = bytes moved
        sns.scatterplot(
            data=data,
            x="Variant",
            y="Bytes_Moved",
            hue="Variant",
            style="Variant",
            s=100,
            ax=ax,
            legend=False  # avoid repeating legend
        )

        ax.set_title(f"{benchmark}", fontsize=10, fontweight='bold')
        ax.set_xlabel("Metric", fontsize=8)
        ax.set_ylabel("Bytes Moved", fontsize=8)
        ax.tick_params(axis='x', rotation=90, labelsize=7)
        ax.set_ylim(bottom=0)
        ax.grid(True, alpha=0.3)

    # Hide empty subplots
    for j in range(i+1, n_rows*n_cols):
        fig.delaxes(axes[j // n_cols, j % n_cols])

    # Add overall title with best metric info
    if best_metric:
        fig.suptitle(f'Memory Traffic Comparison per Benchmark\nBest Metric: {best_metric}', 
                     fontsize=14, fontweight='bold', y=0.995)

    plt.tight_layout()
    plt.savefig("traffic_comparison.pdf", bbox_inches='tight')
    plt.close()
    print("✓ Saved: traffic_comparison.pdf")


def plot_correlation_analysis(df: pd.DataFrame, correlation_results: pd.DataFrame):
    """Scatter plots showing correlation of each metric vs Symbolic Bytes"""
    
    metric_columns = [
        'L3 Misses',
        'L3 DRead + DWrite',
        'L3 DMisses',
        'L3 LST Miss',
        'L3 Access',
        'Max(L1, L2, L3) Misses',
        'LST',
    ]
    
    n_metrics = len(metric_columns)
    n_cols = 3
    n_rows = (n_metrics + n_cols - 1) // n_cols
    
    fig, axes = plt.subplots(n_rows, n_cols, figsize=(6*n_cols, 5*n_rows), squeeze=False)
    
    for i, metric in enumerate(metric_columns):
        ax = axes[i // n_cols, i % n_cols]
        
        # Filter out zeros and NaN
        mask = (df['Symbolic Bytes'] > 0) & (df[metric] > 0)
        x = df.loc[mask, 'Symbolic Bytes']
        y = df.loc[mask, metric]
        
        if len(x) == 0:
            ax.text(0.5, 0.5, 'No valid data', ha='center', va='center', transform=ax.transAxes)
            ax.set_title(f"{metric}", fontsize=10)
            continue
        
        # Scatter plot
        ax.scatter(x, y, alpha=0.6, s=80, edgecolors='black', linewidth=0.5)
        
        # Add diagonal line (perfect correlation)
        max_val = max(x.max(), y.max())
        min_val = min(x.min(), y.min())
        ax.plot([min_val, max_val], [min_val, max_val], 'r--', alpha=0.5, 
                linewidth=2, label='Perfect correlation')
        
        # Get correlation info from results
        if metric in correlation_results.index:
            corr = correlation_results.loc[metric, 'Pearson Correlation']
            r2 = correlation_results.loc[metric, 'R-squared']
            mape = correlation_results.loc[metric, 'MAPE (%)']
            
            # Color code title based on correlation quality
            if corr > 0.9:
                title_color = 'green'
            elif corr > 0.7:
                title_color = 'orange'
            else:
                title_color = 'red'
            
            ax.set_title(f"{metric}\nr={corr:.3f}, R²={r2:.3f}, MAPE={mape:.1f}%", 
                        fontsize=10, color=title_color, fontweight='bold')
        else:
            ax.set_title(f"{metric}", fontsize=10)
        
        ax.set_xlabel("Symbolic Bytes", fontsize=9)
        ax.set_ylabel(f"{metric}", fontsize=9)
        ax.legend(fontsize=8, loc='upper left')
        ax.grid(True, alpha=0.3)
        
        # Use log scale if values span multiple orders of magnitude
        if max_val / min_val > 1000:
            ax.set_xscale('log')
            ax.set_yscale('log')
    
    # Hide empty subplots
    for j in range(i+1, n_rows*n_cols):
        fig.delaxes(axes[j // n_cols, j % n_cols])
    
    fig.suptitle('Correlation Analysis: Hardware Metrics vs Symbolic Bytes', 
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig("correlation_analysis.pdf", bbox_inches='tight')
    plt.close()
    print("✓ Saved: correlation_analysis.pdf")


def plot_correlation_summary(correlation_results: pd.DataFrame):
    """Bar chart summarizing correlations"""
    
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))
    
    # Plot 1: Pearson Correlation
    correlations = correlation_results['Pearson Correlation'].sort_values(ascending=True)
    colors = ['green' if x > 0.9 else 'orange' if x > 0.7 else 'red' for x in correlations]
    
    correlations.plot(kind='barh', ax=ax1, color=colors, edgecolor='black', linewidth=1)
    ax1.set_xlabel('Pearson Correlation Coefficient', fontsize=12, fontweight='bold')
    ax1.set_ylabel('Hardware Metric', fontsize=12, fontweight='bold')
    ax1.set_title('Correlation with Symbolic Bytes', fontsize=14, fontweight='bold')
    ax1.axvline(x=0.9, color='green', linestyle='--', alpha=0.5, linewidth=2, label='Excellent (>0.9)')
    ax1.axvline(x=0.7, color='orange', linestyle='--', alpha=0.5, linewidth=2, label='Good (>0.7)')
    ax1.axvline(x=0.5, color='red', linestyle='--', alpha=0.5, linewidth=2, label='Moderate (>0.5)')
    ax1.legend(fontsize=10)
    ax1.grid(True, alpha=0.3, axis='x')
    ax1.set_xlim(0, 1)
    
    # Plot 2: MAPE (lower is better)
    mape = correlation_results['MAPE (%)'].sort_values(ascending=False)
    colors_mape = ['green' if x < 50 else 'orange' if x < 100 else 'red' for x in mape]
    
    mape.plot(kind='barh', ax=ax2, color=colors_mape, edgecolor='black', linewidth=1)
    ax2.set_xlabel('Mean Absolute Percentage Error (%)', fontsize=12, fontweight='bold')
    ax2.set_ylabel('Hardware Metric', fontsize=12, fontweight='bold')
    ax2.set_title('Prediction Error (Lower is Better)', fontsize=14, fontweight='bold')
    ax2.axvline(x=50, color='green', linestyle='--', alpha=0.5, linewidth=2, label='Good (<50%)')
    ax2.axvline(x=100, color='orange', linestyle='--', alpha=0.5, linewidth=2, label='Moderate (<100%)')
    ax2.legend(fontsize=10)
    ax2.grid(True, alpha=0.3, axis='x')
    
    plt.tight_layout()
    plt.savefig("correlation_summary.pdf", bbox_inches='tight')
    plt.close()
    print("✓ Saved: correlation_summary.pdf")


def plot_flops(df: pd.DataFrame):
    """Original FLOPs comparison plot"""
    
    # Columns to compare
    traffic_columns = ['PAPI FLOPs', 'work']

    # Melt the dataframe for plotting
    df_melt = df.melt(
        id_vars=["benchmark"], 
        value_vars=traffic_columns,
        var_name="Variant",
        value_name="Value"   # must not match existing column names
    )

    # Plot setup
    sns.set(style="whitegrid")
    benchmarks = df["benchmark"].unique()
    n_cols = 3
    n_rows = (len(benchmarks) + n_cols - 1) // n_cols

    fig, axes = plt.subplots(n_rows, n_cols, figsize=(5*n_cols, 4*n_rows), squeeze=False)

    for i, benchmark in enumerate(benchmarks):
        ax = axes[i // n_cols, i % n_cols]
        data = df_melt[df_melt["benchmark"] == benchmark]

        # Scatter plot: x = variant, y = value
        sns.scatterplot(
            data=data,
            x="Variant",
            y="Value",
            hue="Variant",
            style="Variant",
            s=100,
            ax=ax,
            legend=False  # avoid repeating legend
        )

        ax.set_title(f"{benchmark}", fontsize=10, fontweight='bold')
        ax.set_xlabel("Metric", fontsize=8)
        ax.set_ylabel("FLOPs / Work", fontsize=8)
        ax.tick_params(axis='x', rotation=45, labelsize=7)
        ax.set_ylim(bottom=0)
        ax.grid(True, alpha=0.3)

    # Hide empty subplots
    for j in range(i+1, n_rows*n_cols):
        fig.delaxes(axes[j // n_cols, j % n_cols])

    fig.suptitle('FLOPs Comparison per Benchmark', fontsize=14, fontweight='bold', y=0.995)
    plt.tight_layout()
    plt.savefig("flops_comparison.pdf", bbox_inches='tight')
    plt.close()
    print("✓ Saved: flops_comparison.pdf")


if __name__ == "__main__":
    main()